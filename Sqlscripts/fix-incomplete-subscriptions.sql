-- Script para arreglar suscripciones incompletas
...
-- (Solo si tienes pagos exitosos correspondientes)
...
WHERE id IN (
  ...
  JOIN payment_history ph ON us.user_id = ph.user_id 
    AND us.subscription_plan_id = ph.subscription_plan_id
  WHERE us.status = 'incomplete' 
    AND ph.status = 'succeeded'
    AND ph.created_at >= us.created_at - INTERVAL '5 minutes'
);
